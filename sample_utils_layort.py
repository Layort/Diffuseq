...
class GSN_matrix(nn.Module):
    def __init__(self,args, n_batch ,n_nodes,n_token,n_token_dim, vocab_size):
        cuda = th.device(f"cuda:{os.environ['LOCAL_RANK']}")
        cpu = th.device('cpu')
    ...
    def forward(self,group_lst):
       ...
       # dec_hidden_state_init 为要生成那句话的sentence embedding th.Size([200, 300])
        ...
        for i, output in enumerate(dec_out):
            vocab_scores.append(self.out_linear(output))
        vocab_dists = [F.softmax(s) for s in vocab_scores]
        self.output_eval = F.softmax(th.cat(vocab_scores, dim = 1))


        # mask = (self.target_batch != self.padding_mark).float()
        # self.seq_loss = F.cross_entropy(vocab_scores, self.target_batch, ignore_index=self.padding_mark, reduction='none')
        # self.seq_loss *= mask  # Apply the mask to ignore padding positions
        # self.seq_loss = self.seq_loss.sum(dim=1) / mask.sum(dim=1)  # Normalize the loss by the number of non-padding positions in each sequence
        # loss = self.seq_loss.mean()
    ...
    def concat_conv(self,args,  matrix , bias  ):
        if len(args) == 1:
