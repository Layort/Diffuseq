...
def attention_decoder(decoder_inputs,
    ...
    encoder_features = F.conv2d(encoder_states, W_h, stride=1, padding="same")
    ...
    decoder_inputs = th.split(decoder_inputs, 1, dim=1).tolist()#返回是元组
    decoder_inputs = th.squeeze(decoder_inputs, dim=1)
...
def concat_conv(self,args,  matrix , bias , output_size ):
        if len(args) == 1:
