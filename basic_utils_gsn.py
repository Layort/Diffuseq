...
from diffuseq_layort import gaussian_diffusion_layort as gd
from diffuseq_layort.gaussian_diffusion_layort import SpacedDiffusion, space_timesteps
from diffuseq_layort.transformer_model import TransformerNetModel
...
class myTokenizer():
    ...
    def __init__(self, args):
        if args.vocab == 'bert':
            ...
            print('#'*30, 'load vocab from', path_save_vocab)
            vocab_dict = {'[START]': 0, '[END]': 1, '[UNK]':2, '[PAD]':3}
            if(args.vocab == 'to_design'):
                with open(path_save_vocab, 'r', encoding='utf-8') as f:
                    for row in f:
                        vocab_dict[row.strip().split(' ')[0]] = len(vocab_dict)
            else:
                if(args.vocab == 'designed' and 'top_p' in args):
                    with open(path_save_vocab, 'r', encoding='utf-8') as f:
                        for row in f:
                            vocab_dict[row.strip().split(' ')[0]] = len(vocab_dict)
...
def load_defaults_config():
    ...
    """
    with open('diffuseq_layort/config.json', 'r') as f:
        return json.load(f)
