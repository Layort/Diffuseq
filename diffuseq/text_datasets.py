...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    def tokenize_function(examples):
        ...
        print('len(input_id_x[0])  ', len(input_id_x[0]),' len(input_id_x[0])  ', len(input_id_y[0]))
        return result_dict
...
def get_corpus(data_args, seq_len, split='train', loaded_vocab=None):
    ...
    with open(path, 'r') as f_reader:
        for row in f_reader:
            ...
            sentence_lst['src'].append(src.strip())
            sentence_lst['trg'].append(json.loads(row)['answer'].strip())
