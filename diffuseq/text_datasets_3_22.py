...
def helper_tokenize_sample(sentence_lst, vocab_dict, seq_len):
    ...
    print('len(tokenized_datasets_input_ids)', len(tokenized_datasets['input_ids']))
    print('len(tokenized_datasets_input_ids[0])', len(tokenized_datasets['input_ids'][0]))
    print('### tokenized_datasets', tokenized_datasets)
    ...
    def split_uttrance2(group_lst):
        ...
        mask = []
        token_dim = 16
        res= torch.full( ( len(group_lst['input_id_x']) ,MAX_UTTERANCE_NUM-1, MAX_UTTERANCE_TOKEN, token_dim), -1)
        middle =torch.zeros(( len(group_lst['input_id_x']),MAX_UTTERANCE_NUM-1,2),dtype = torch.int16 )
        middle[:,:,0]   = torch.tensor(group_lst['index_lst'][:,:-1]) #  0, 16, 87, 99, 198
        middle[:,:,1] = torch.tensor(group_lst['index_lst'][:,1:])#  16, 87, 99, 198 ,0
        middle[:,-1,1] = -1  # 16,87, 99, 198, -1

        res[:,:,:,:] = group_lst['input_id_x'][ middle[:,:,0], middle[:,:,1]]

        for i in range(len(group_lst['input_id_x'])):
            for j in  range(len(group_lst['index_lst'][i])):
                 res[i,j,0:middle[i,j][1]-middle[i,j][0]] = group_lst['input_id_x'][i,j][middle[i,j][0]:middle[i,j][1]]
        
        group_lst['input_ids'] = res
        return group_lst
    ...
    print('len(tokenized_datasets_input_split_ids)', len(tokenized_datasets['input_split_ids']))
    print('len(tokenized_datasets_input_split_ids[0])', len(tokenized_datasets['input_split_ids'][0]))
...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    def merge_and_mask(group_lst):
        ...
        for i in range(len(group_lst['input_id_x'])):
            ...
            mask.append([1] * (len(src) + 1))  # 这里从0 改成了 1
        group_lst['input_ids'] = lst
        group_lst['input_mask'] = mask
        return group_lst
    ...
    if (split == 'train'):  # 首先自定义字表加载是在train， valid和test不重新制作，只是加载
        ...
        with open(path, 'r') as f_reader:
            for row in f_reader:
                ...
                print(relation_at)
                print(type(relation_at))
                print(index_lst)
                print(type(index_lst))
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with torch.no_grad():
            ...
            out_kwargs['input_split_ids'] = np.array(self.text_datasets['train'][idx]['input_split_ids'])
