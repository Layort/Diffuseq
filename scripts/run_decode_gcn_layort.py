...
if __name__ == '__main__':
    ...
    for lst in glob.glob(args.model_dir):
        ...
        for checkpoint_one in checkpoints:
            ...
            COMMAND = f'python -m torch.distributed.launch --nproc_per_node=1 --master_port={12233 + int(args.seed)} --use_env sample_gcn_layort.py ' \
                      f'--model_path {checkpoint_one} --step {args.step} ' \
                      f'--batch_size {args.bsz} --seed2 {args.seed} --split {args.split} ' \
                      f'--out_dir {out_dir} --top_p {args.top_p} --use_norm {args.use_norm} --alpha {args.alpha} --dropout {args.dropout}'
            print(COMMAND)
