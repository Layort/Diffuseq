...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='train_gcn args.')
    ...
    parser.add_argument('--save_interval', type=int, default=50, help='save step')
    parser.add_argument('--resume_checkpoint', type=str, default='none',help='path to resume checkpoint, like xxx/xxx.pt')

    parser.add_argument('--use_norm', type=bool, default=True, help='whtether to use norm')
    parser.add_argument('--dropout', type=float, default=0.2, help='dropout rate')
    parser.add_argument('--alpha', type=float, default=0.02, help='alpha to normize')
    parser.add_argument('--epoches', type=int, default=50, help='times to train')
    parser.add_argument('--sub_seq_len', type=int, default=90, help='length of sub untterance')
    parser.add_argument('--microbatch', type=int, default=64, help='length of micro_batch')
    ...
    folder_name = 'gsn_models/'
    if int(os.environ['LOCAL_RANK']) == 0:
        if not os.path.isdir(folder_name):
            os.mkdir(folder_name)

    Model_FILE = f"gcn_{args.dataset}_h{args.hidden_dim}_lr{args.lr}" \
                f"_ep{args.epoches}_sublen{args.sub_seq_len}" \
                f"_seed{args.seed}"
    ...
    seq_checkpoint = sorted(glob.glob(f"{args.model_dir}/{args.pattern}*.pt"))[-1]
    
    COMMAND =   f"OPENAI_LOGDIR={Model_FILE}  " \
                f"TOKENIZERS_PARALLELISM=false " \
                f'python train_gsn.py  ' \
                f"--checkpoint_path {Model_FILE} " \
                f'--model_path {seq_checkpoint} --step {args.step} ' \
                f'--batch_size {args.bsz} --microbatch {args.microbatch} --seed2 {args.seed} --split {args.split} ' \
                f'--top_p {args.top_p} --lr {args.lr} --alpha {args.alpha} ' \
                f'--dropout {args.dropout} --use_norm {args.use_norm} --epoches {args.epoches} ' \
                f"--sub_seq_len {args.sub_seq_len} " 

    print(COMMAND)
    os.system(COMMAND)
    
    print('#'*30, 'training gsn finished...')
...
#                   f"--use_norm {args.use_norm} --epoches {args.epoches}"
