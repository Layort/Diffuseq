...
def langevin_fn_gsn(model_gsn, need, sample, mean, sigma, alpha, t, pre_sample, group_lst, mask):
    ...
    sample_y = th.broadcast_to(group_lst['pad_embed'],
                               (model_gsn.n_batch, model_gsn.n_token, model_gsn.n_token_dim)).clone()
...
class GSN_matrix(nn.Module):
    def __init__(self, args, n_batch, n_nodes, n_token, n_token_dim):
        ...
        self.gru_fwd = nn.GRUCell(input_size=self.n_lstm * 2, hidden_size=self.n_lstm * 2, bias=True, device=cuda)
        self.drop_fwd = nn.Dropout(args.dropout)
        self.gru_bwd = nn.GRUCell(input_size=self.n_lstm * 2, hidden_size=self.n_lstm * 2, bias=True, device=cuda)
        ...
        self.full1 = nn.Sequential(nn.Linear(self.n_lstm * 2, self.n_lstm), nn.ReLU())
        self.GetGPUInfo = GetGPUInfo((0, 1, 2, 3))

        self.bi_lstm = nn.LSTM(input_size=self.n_token_dim, hidden_size=self.n_lstm, num_layers=1, batch_first=True,
                               bidirectional=True)
        # self.full2 = nn.Sequential(nn.Linear(n_token_dim , n_token_dim ), nn.ReLU())
    ...
    def _build_matirx(self, group_lst):
        ...
        # encoder_ouput.shape = N, n_token , 2*hidden_size
        encoder_ouput, (fw_st, bw_st) = self.bi_lstm(encode_x_lst)

        # ouput.shape N*n_nodes,n_token,n_token_dim
        ...
        try:
            sen_enc_states = th.cat([positonal_encoding, encoder_ouput])
    ...
    def _compute_update_info(self, group_lst):
        ...
        # 进入全连接层，Relu激活
        hidden_state_list = self.full1(hidden_state_list.reshape(self.n_batch * self.n_nodes + 1, self.n_lstm * 2))
    ...
    def forward(self, group_lst):
        ...
        group_lst = self._compute_update_info(group_lst)
        tgt_hidden_state = th.index_select(group_lst['hidden_state_list'], 0, group_lst['dec_idx'])
        ...
        assert (tgt_hidden_state.shape == (self.n_batch, self.n_lstm))

        tgt_hidden_state = tgt_hidden_state.reshape(self.n_batch, self.n_lstm)

        # print("tgt_hidden_state[:,:,10:20]",tgt_hidden_state[:10,:,10:20])
