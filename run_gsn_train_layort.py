...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoches', type=int, default=1000, help='times to train')
    ...
    checkpoint = sorted(glob.glob(f"{args.model_dir}/{args.pattern}*.pt"))[-1]
    
    COMMAND =   f"OPENAI_LOGDIR={args.model_dir}  " \
                f"TOKENIZERS_PARALLELISM=false " \
                f'python train_gsn.py  '\
                f'--model_path {checkpoint} --step {args.step} ' \
