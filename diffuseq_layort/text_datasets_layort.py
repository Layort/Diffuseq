...
from . import MAX_UTTERANCE_NUM, MAX_UTTERANCE_TOKEN
...
def get_corpus(data_args, seq_len, sub_seq_len, split='train', loaded_vocab=None):
    ...
    sentence_lst = {'src': [], 'trg': [], 'relation_at': [], 'history': [], 'dec_idx':[],'tgt_idx':[]}
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with torch.no_grad():
            ...
            out_kwargs['input_split_id_x'] = torch.tensor(self.text_datasets['train'][idx]['input_split_id_x'])
            out_kwargs['relation_at'] = torch.tensor(self.text_datasets['train'][idx]['relation_at']).long()
            out_kwargs['dec_idx'] = torch.tensor(self.text_datasets['train'][idx]['dec_idx']).long()
...
def _collate_batch_helper_split(examples, pad_token_id, n_token, return_mask=False):
    result_split = torch.full([len(examples), MAX_UTTERANCE_NUM, n_token], pad_token_id,
                              dtype=torch.int64).tolist()

    for i, sentence in enumerate(examples):
        for j, sub_sen in enumerate(sentence):
            sub_sen_token = min(len(sub_sen), n_token)
